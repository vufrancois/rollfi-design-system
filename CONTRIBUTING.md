...
1. **Tokens over hardcoded values.** Every color, spacing value, radius, shadow, and type style comes from a `--rf-*` CSS custom property. If a token doesn't exist for what you need, add one in `src/tokens/`. **Critical for white-labelling**: any new component that uses a brand or sidebar accent MUST consume `--rf-color-brand*` / `--rf-color-sidebar*` tokens (never raw hex like `#FF2F1C` or `#ffffff` on sidebar surfaces) — that's the only way per-tenant `<BrandProvider>` overrides flow through automatically. **Sidebar-specific reminder**: when rendering content into Sidebar's `logo`, `footer`, or custom slots, use `--rf-color-sidebar-text` / `--rf-color-sidebar-text-dim` / `--rf-color-sidebar-text-strong` rather than `#fff` or `rgba(255,255,255,…)`; otherwise the slot disappears under a light-mode tenant theme.
